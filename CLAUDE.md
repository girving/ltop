...
24. **`<[T]>::contains` on integer slices is not a loop.** It dispatches to the `SliceContains` specialisation: a `chunks_exact(16)` branchless OR-accumulate pass plus a remainder loop — ~104 B on arm64, NEON-vectorised on Linux aarch64 (452 B there). Built for large slices; our tables are 17–18 entries. `bytes::has` (`t.iter().any(|&x| x == h)`) is 10 instructions. Same family as lesson 2: a stdlib method whose fast path is a specialisation you didn't ask for. `<[u8]>::contains` goes to `memchr` the same way; `map::Set::contains` is `binary_search` and fine. Corollary on shared helpers: when two call sites pass the same constant argument, IPSCCP folds it into the body and the `mov`s at both sites die; make them differ and each site pays 4 B — so a "redundant" entry that keeps two tables the same length can be accidentally load-bearing. Measure the removal.
25. **Slice `==` against a byte-string literal unrolls into immediate chains.** `name == b"lean --server"` has a constant length, so LLVM emits the compare as unaligned loads assembled with `movzwl`/`shl`/`or` against 10-byte `movabs` immediates — ~75 B per compare on x86_64, and every new literal pays again. `bytes::ieq(name, b"…")` is a 68 B loop that's already linked; each call site costs a `lea`+`mov`+`call` (~20 B) and the literal lives in `.rodata` at its own length. The `coalesce_root` `lean --server` fold went +152 B → +80 B this way. Same for `starts_with`/`ends_with` with a literal: they stay out of line only because both call sites pass a runtime length. Order the tests so the cheap-and-usually-false one gates the rest; a bool hoisted out of the loop to skip a compare is a loop-carried spill (~13 B) that a lazy `&&` avoids.
